...
async function seedUsers(roles: { id: string; title: string; description: string; createdAt: Date; updatedAt: Date | null; deletedAt: Date | null }[]) {
  ...
  for (let i = 1; i < CONFIG.USERS_COUNT; i++) {
    ...
    const firstName = faker.person.firstName().substring(0, 50)
    const lastName = faker.person.lastName().substring(0, 50)
    ...
    usersData.push({
      ...
      password: hashedPassword,
      firstname: firstName,
      othernames: `${lastName} ${faker.person.middleName().substring(0, 20)}`.substring(0, 100),
      dateOfBirth: faker.date.birthdate({ min: 18, max: 65, mode: 'age' }).toISOString().split('T')[0],
      gender,
      nationalId: faker.string.alphanumeric(15).toUpperCase(),
      phone: faker.phone.number().substring(0, 20),
      nextOfKin: faker.person.fullName().substring(0, 100),
      nextOfKinContacts: faker.phone.number().substring(0, 100),
...
async function seedEvents(users: { id: string; roleId: string; email: string; role: { id: string; title: string } }[]) {
  ...
  for (let i = 0; i < CONFIG.EVENTS_COUNT; i++) {
    ...
    eventsData.push({
      title: `${faker.company.catchPhrase()} ${faker.date.recent().getFullYear()}`.substring(0, 255),
      description: faker.lorem.paragraphs(2, '\n\n').substring(0, 1000),
      date: startDate,
      location: `${faker.location.city()}, ${faker.location.country()}`.substring(0, 255),
...
async function seedRegistrations(events: { id: string; title: string; capacity: number }[]) {
  ...
  for (const event of events) {
    ...
    for (let i = 0; i < registrationCount; i++) {
      let email = faker.internet.email().substring(0, 100)
      ...
      registrationsData.push({
        ...
        email,
        affiliation: faker.company.name().substring(0, 100),
        position: faker.person.jobTitle().substring(0, 100),
        country: faker.location.country().substring(0, 50),
        phoneNo: faker.phone.number().substring(0, 20),
        passportNumber,
        passportExpiryDate: faker.date.future({ years: 5 }),
        departureCity: `${faker.location.city()}, ${faker.location.country()}`.substring(0, 100),
